...
DB_CONFIG_DEFAULT: Dict[str, Any] = {
    "db_type": "SQLite",
    "db_driver": "aiomysql",
    "db_custom_url": "",
    "db_host": "localhost",
...
def _load_db_config() -> Dict[str, Any]:
    """读取 configs/database_config.json, 缺失项回落默认值。"""
    conf = dict(DB_CONFIG_DEFAULT)
    path = get_res_path("configs") / "database_config.json"
    if not path.exists():
        return conf
    ...
    try:
        ...
        logger.warning(f"[数据库] 配置读取失败, 使用默认配置: {e}")
        return conf

    if not isinstance(raw, dict):
        return conf

    for key in DB_CONFIG_DEFAULT:
        if key not in raw:
            continue
        value = raw[key]
        # 配置文件既可能是裸值, 也可能是 {"data": 值} 的配置项结构
        if isinstance(value, dict):
            value = value.get("data", conf[key])
        if value is not None:
            conf[key] = value
    return conf


_db_config_data = _load_db_config()

db_host: str = _db_config_data["db_host"]
db_port: str = _db_config_data["db_port"]
db_user: str = _db_config_data["db_user"]
db_password: str = _db_config_data["db_password"]
db_name: str = _db_config_data["db_name"]

db_pool_size: Optional[int] = _db_config_data["db_pool_size"]
db_echo: bool = _db_config_data["db_echo"]
db_pool_recycle: int = _db_config_data["db_pool_recycle"]

db_custom_url = _db_config_data["db_custom_url"]
db_type: str = _db_config_data["db_type"]

db_driver: str = _db_config_data["db_driver"]
...
DB_PATH = get_res_path() / "WavesData.db"
...
async def init_database():
    ...
    async with _db_init_lock:
        ...
        try:
            if _db_type == "sqlite":
                # sqlite 显式 NullPool, 并发上限由下方 semaphore 控制
                db_config.update(
                    {
            ...
            logger.exception(f"[数据库] 连接失败: {e}")
            raise ValueError(f"[数据库] [{base_url}] 连接失败, 请检查配置文件!")
...
def with_session(
    ...
    async def wrapper(self, *args: P.args, **kwargs: P.kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if _is_pool_timeout(e):
                    logger.error(f"[数据库] 连接池耗尽/超时, 停止重试: {e}")
                    raise
        ...
        if last_err is not None:
            raise last_err
        raise RuntimeError("[数据库] with_session 未知失败")
...
class BaseIDModel(SQLModel):
    ...
    async def batch_insert_data_with_update(
        ...
        if _db_type == "sqlite":
            ...
            raise ValueError(f"[数据库] 不支持 {_db_type} 数据库!")
    ...
    def get_gameid_name(cls, game_name: Optional[str] = None) -> str:
        ...
        🌱参数:
            ...
            🔹game_name (`Optional[str]`, 默认是 `None`):
                    ...
                    否则在校验阶段抛出 ``ValueError``。

        🚀使用范例:

            `await WavesUser.get_gameid_name('sr')`
    ...
    async def full_insert_data(cls, session: AsyncSession, **data) -> int:
        ...
        🚀使用范例:
            ...
            `await WavesUser.full_insert_data(uid='123',cookie='233', ...)`
    ...
    async def delete_row(
        ...
        **data,
        ...
        logger.trace(f"[数据库] 即将删除{row_data}")
        if row_data:
            for row in row_data:
    ...
    async def select_rows(
        ...
        **data,
        ...
        🚀使用范例:
            ...
            `await WavesUser.base_select_data(uid='100740568')`
    ...
    async def data_exist(cls, **data) -> bool:
        ...
        🚀使用范例:
            ...
            `await WavesUser.data_exist(uid='100740568')`
    ...
    async def update_data_by_uid_without_bot_id(
        ...
        **data,
        ...
        🚀使用范例:
            ...
            `await WavesUser.update_data_by_uid_without_bot_id(uid, cookie='2')`
    ...
    async def update_data_by_xx(
        ...
        **data,
        ...
        🚀使用范例:
            ...
            `await WavesUser.update_data_by_xx({'uid': '233'}, cookie=ck)`
    ...
    async def update_data_by_uid(
        ...
        **data,
        ...
        🚀使用范例:
            ...
            `await WavesUser.update_data_by_uid(uid, 'onebot', cookie='2')`
...
class BaseModel(BaseBotIDModel):
    ...
    async def select_data_list(
        ...
        🚀使用范例:
            ...
            `await WavesUser.select_data(user_id='444888', bot_id='onebot')`
    ...
    async def insert_data(
        ...
        **data,
        ...
        🚀使用范例:
            ...
            `await WavesUser.insert_data(user_id='4', bot_id='onebot', uid='22')`
    ...
    async def delete_data(
        ...
        **data,
        ...
        🚀使用范例:
            ...
            `await WavesUser.delete_data(user_id='4', bot_id='onebot', uid='22')`
    ...
    async def update_data(
        ...
        **data,
        ...
        🚀使用范例:
            ...
            `await WavesUser.update_data(user_id='4', bot_id='onebot', uid='22')`
...
class Bind(BaseModel):
    ...
    async def get_uid_list_by_game(
        ...
        🚀使用范例:
            ...
            `await WavesUser.get_uid_list_by_game(user_id='4', bot_id='onebot')`
    ...
    async def get_uid_by_game(
        ...
        🚀使用范例:
            ...
            `await WavesUser.get_uid_by_game(user_id='4', bot_id='onebot')`
    ...
    async def insert_uid(
        ...
        🚀使用范例:
            ...
            `await WavesBind.insert_uid(qid, ev.bot_id, uid, ev.group_id, 9)`
    ...
    async def delete_uid(
        ...
        🚀使用范例:
            ...
            `await WavesBind.delete_uid(qid, ev.bot_id, uid)`
    ...
    async def get_all_uid_list_by_game(
        ...
        🚀使用范例:
            ...
            `await WavesBind.get_all_uid_list_by_game(ev.bot_id)`
    ...
    async def switch_uid_by_game(
        ...
        🚀使用范例:
            ...
            `await WavesBind.switch_uid_by_game(qid, ev.bot_id, uid)`
...
class User(BaseModel):
    ...
    async def select_data_by_uid(
        ...
        🚀使用范例:
            ...
            `await WavesUser.select_data_by_uid(uid='100740568')`
    ...
    async def get_user_all_data_by_user_id(cls: Type[T_User], session: AsyncSession, user_id: str):
        ...
        🚀使用范例:
            ...
            `await WavesUser.get_user_all_data_by_user_id(user_id='2333')`
    ...
    async def get_switch_open_list(cls: Type[T_User], session: AsyncSession, switch_name: str) -> List[T_User]:
        ...
        🚀使用范例:
            ...
            `await WavesUser.get_switch_open_list('sign_switch')`
    ...
    async def get_random_cookie(
        ...
        🌱参数:
            ...
            🔹cache_model (`Optional[Type["Cache"]]`, 默认是 `None`):
                    继承基础`Cache`缓存表的模型, 例如`WavesCache`
        ...
        🚀使用范例:
            ...
            `await WavesUser.get_random_cookie(
            uid, WavesCache, {'region': server}, game_name='sr'
        )`
...
class Push(BaseBotIDModel):
    ...
    async def select_data_by_uid(
        ...
        🚀使用范例:
            ...
            `await WavesPush.select_data_by_uid(uid='100740568')`
